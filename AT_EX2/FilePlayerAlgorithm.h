...
class FilePlayerAlgorithm : public FilePlayerAlgorithm{
public:
    ...
    // don't need implementation foor file player - Does not important
    virtual void notifyOnInitialBoard(const Board& b, const std::vector<unique_ptr<FightInfo>>& fights) {}
    virtual void notifyOnOpponentMove(const Move& move) = 0; // called only on opponents move
